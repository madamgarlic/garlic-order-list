...
# 정제 기준 키워드
품종_키워드 = ["육쪽", "대서"]
형태_키워드 = ["다진마늘", "깐마늘", "통마늘", "무뼈닭발", "마늘빠삭이", "마늘쫑"]
크기_키워드 = ["특", "대", "중", "소"]
꼭지_키워드 = ["꼭지제거", "꼭지포함"]
...
if uploaded_files:
    ...
    if 정제_전체:
        ...
        for _, row in df_all.iterrows():
            ...
            if is_업소용:
                key = (옵션, 단위)
                grouped_rows[key] = grouped_rows.get(key, 0) + row["총수량"]
            elif cat in ["마늘", "마늘쫑"]:
                # 무게 포함된 옵션 제거 (정제옵션명 단일화)
                base_옵션 = " ".join(옵션.split()[:-1])
                key = (base_옵션, 단위)
                grouped_rows[key] = grouped_rows.get(key, 0) + row["총중량(kg)"]
            else:
                # 무뼈닭발, 빠삭이
                key = (옵션, 단위)
                grouped_rows[key] = grouped_rows.get(key, 0) + row["총수량"]
        ...
        df_summary = pd.DataFrame(
            [(opt, unit, round(qty)) for (opt, unit), qty in grouped_rows.items()],
