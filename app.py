...
if uploaded_files:
    for file in uploaded_files:
        ...
        for col in df.columns:
            if any(k in str(col).lower() for k in ['옵션', '옵션명', '옵션정보', '상품명']):
                option_column = col
                break
        ...
        # 요약
        summary = df.groupby('정제된옵션명', as_index=False).agg({
            '총수량': 'sum',
            '총중량(kg)': 'sum',
            '단위무게(g)': 'first'
        })

        summary = summary.rename(columns={'총수량': '🔢 총수량', '총중량(kg)': '📦 총중량(kg)', '단위무게(g)': '단위무게(g, 참고)'})
        ...
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            summary.to_excel(writer, index=False, sheet_name='정제결과')
        st.download_button(
            label="📥 엑셀로 다운로드",
            data=buffer.getvalue(),
            file_name=f"정제옵션_{file.name.replace('.xlsx', '')}.xlsx",
