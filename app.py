...
if uploaded_files:
    ...
    for file in uploaded_files:
        ...
        all_rows.append(df[['정제된옵션명', '총수량', '총중량(kg)', '단위무게(g)', 'is_업소용', '카테고리']])

        df_download = df.copy()
        df_download[option_column] = df_download['정제된옵션명']
        buffer = io.BytesIO()
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            df_download.drop(columns=['정제된옵션명', '포장수량', '단위무게(g)', '총중량(kg)', 'is_업소용', '카테고리']).to_excel(writer, index=False)
        st.download_button(
    ...
    if all_rows:
        ...
        df_업소용 = df_all[df_all['is_업소용']]
        df_닭발빠삭 = df_all[df_all['카테고리'].isin(['빠삭이', '닭발'])]
        df_중량합산 = df_all[(~df_all['is_업소용']) & (~df_all['카테고리'].isin(['빠삭이', '닭발']))]

        summary1 = df_업소용.groupby('정제된옵션명', as_index=False).agg({
            '총수량': 'sum',
            '총중량(kg)': 'sum',
            '단위무게(g)': 'first'
        }).rename(columns={
        ...
        summary2 = df_닭발빠삭.groupby('정제된옵션명', as_index=False).agg({
            '총수량': 'sum'
        }).rename(columns={'총수량': '🔢 총수량'})

        summary3 = df_중량합산.groupby('정제된옵션명', as_index=False).agg({
            '총중량(kg)': 'sum'
        }).rename(columns={'총중량(kg)': '📦 총중량(kg)'})

        summary = pd.concat([summary1, summary2, summary3], ignore_index=True)
        st.markdown("### ✅ 최종 패킹리스트")
        st.dataframe(summary)

        buffer2 = io.BytesIO()
        with pd.ExcelWriter(buffer2, engine='openpyxl') as writer:
            summary.to_excel(writer, index=False, sheet_name='패킹리스트')
        st.download_button(
            label="⬇ 최종 패킹리스트 다운로드",
